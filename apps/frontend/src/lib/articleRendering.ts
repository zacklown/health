...
export function buildDictionaryAnchor(slug: string): string {
  ...
  const letter = getDictionaryLetter(normalized)
    .toLowerCase()
...
function renderTermLinks(text: string): string {
  const bracketSyntaxLinked = text.replace(
    /\[\[([a-z0-9][a-z0-9-]{0,63})\]\]/gi,
    (match, rawTag) => {
  ...
  return bracketSyntaxLinked.replace(
    /(^|[\s(>])#([a-z0-9][a-z0-9-]{0,63})\b/gi,
    (match, prefix, rawTag) => {
      const tag = String(rawTag || '')
      if (!tag) return match
...
export function renderMarkdownWithToc(markdown: string): { html: string; toc: TocItem[] } {
  ...
  for (const rawLine of lines) {
    ...
    if (headingMatch) {
      ...
      htmlParts.push(
        `<h${level} id="${headingId}">${renderInlineMarkdown(headingText)}</h${level}>`,
      )
      continue
    }
