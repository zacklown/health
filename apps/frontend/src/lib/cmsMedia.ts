...
export function getPublicCmsBaseUrl(
	...
	return env.PUBLIC_CMS_PUBLIC_URL?.replace(/\/+$/, '') || `${siteUrl.protocol}//${siteUrl.hostname}:3000`;
}
	...
	if (/^https?:\/\//i.test(url)) {
		try {
			...
			const isInternalCmsHost =
				INTERNAL_CMS_HOSTS.has(parsed.hostname) || (parsed.port === '3000' && parsed.hostname !== '');
